...
model TEDS_ExpTestInputs_ControlsTestingDMM_May2025Update
  "Test designed to ensure the TEDS loop can operate in all modes."

  parameter Real FV_opening=0.00250;
  parameter Modelica.Units.SI.CoefficientOfHeatTransfer hc_piping = 15;
  parameter Modelica.Units.SI.Temperature T_air = 20+273.15;
  ...
  Modelica.Fluid.Pipes.DynamicPipe pipe2(
    ...
    redeclare model HeatTransfer =
        Modelica.Fluid.Pipes.BaseClasses.HeatTransfer.IdealFlowHeatTransfer (k=
            hc_piping, T_ambient=T_air))
    "Length taken from TEDS Installation Diagram detailed # 13, measured from MOS interface to the T junction. "
  ...
  Modelica.Fluid.Pipes.DynamicPipe pipe5(
    ...
    redeclare model HeatTransfer =
        Modelica.Fluid.Pipes.BaseClasses.HeatTransfer.IdealFlowHeatTransfer (k=
            hc_piping, T_ambient=T_air))
    "Length taken from TEDS as-buiilt, detailed diagram #12. Note that this pipe actually makes pipe 10 functionally redundant. "
  ...
  SupportComponents.NonLinear_Break
    ...
    annotation (Placement(transformation(extent={{6,-8},{-6,8}},
        rotation=90,
        origin={214,-102})));
  TRANSFORM.Fluid.Sensors.TemperatureTwoPort TC_001(redeclare package Medium =
        TRANSFORM.Media.Fluids.Therminol_66.LinearTherminol66_A_250C, precision=
       3) annotation (Placement(transformation(
        extent={{-5,-4},{5,4}},
        rotation=180,
...
equation
  ...
  connect(T_chiller_before.port_b, pipe7.port_a)
    annotation (Line(points={{214,2},{214,-10},{212,-10}},
                                                 color={0,127,255}));
  connect(pipe7.port_b, T_chiller_after.port_a) annotation (Line(points={{212,-26},
          {212,-40},{214,-40},{214,-52}},
                                     color={0,127,255}));
  ...
  connect(PV_004.port_b, ValveFl.port_b)
    annotation (Line(points={{-84,-146},{-72,-146}}, color={0,127,255}));
  ...
  connect(pipe3.port_b, nonLinear_Break3.port_a)
    annotation (Line(points={{198,-144},{118,-144}}, color={0,127,255}));
  connect(pipe8.port_b, Chiller_Mass_flow_T66.port_a) annotation (Line(points={{140,
          -186},{124.5,-186},{124.5,-190}},     color={0,127,255}));
  connect(PV_052.port_b, pipe8.port_a) annotation (Line(points={{174,-136},{174,
          -186},{152,-186}}, color={0,127,255}));
  connect(PV_008.port_b, pipe8.port_a) annotation (Line(points={{190,-182},{190,
          -186},{152,-186}}, color={0,127,255}));
  connect(PV_051.port_a, pipe9.port_a) annotation (Line(points={{62,-106},{90,-106}},
                      color={0,127,255}));
  connect(pipe9.port_b, T_ch_o.port_b) annotation (Line(points={{102,-106},{114,
  ...
  connect(Ac.Valve_fl, ValveFl.opening) annotation (Line(
      points={{-19,142},{-19,88},{-10,88},{-10,60},{240,60},{240,-260},{-66,
          -260},{-66,-150.8}},
      color={239,82,82},
      pattern=LinePattern.Dash,
      thickness=0.5), Text(
  ...
  connect(nonLinear_Break5.port_b, pipe3.port_a) annotation (Line(points={{214,-108},
          {214,-144},{210,-144}},       color={0,127,255}));
  connect(PV_008.port_a, pipe3.port_b) annotation (Line(points={{190,-170},{190,
          -144},{198,-144}}, color={0,127,255}));
  connect(ValveFl.port_a, TC_001.port_b)
    annotation (Line(points={{-60,-146},{-56,-146}}, color={0,127,255}));
  connect(TC_001.port_a, FM_001.port_b)
    annotation (Line(points={{-46,-146},{-42,-146}}, color={0,127,255}));
  annotation (
...
end TEDS_ExpTestInputs_ControlsTestingDMM_May2025Update;
