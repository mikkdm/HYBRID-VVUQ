...
model TEDS_ExpSimMay2025
  "Test designed to ensure the TEDS loop can operate in all modes."

  parameter Real FV_opening=0.00250;
  parameter Modelica.Units.SI.CoefficientOfHeatTransfer hc_air = 75;
  parameter Modelica.Units.SI.Temperature T_ambient = 20+273.15;
  ...
  TRANSFORM.Fluid.Sensors.MassFlowRate FM_003(redeclare package Medium =
       ...
       3) annotation (Placement(transformation(
        ...
        rotation=270,
        origin={-103,-98})));
  UCA.Controls.Control_System_TEDS_ExpTest_Oct2024_chargeto80percent
  ...
  BaseClasses_1.SignalSubBus_ActuatorInput Sen
    annotation (Placement(transformation(extent={{-84,130},{-62,154}}),
        iconTransformation(extent={{-84,130},{-62,154}})));
  ...
  Modelica.Fluid.Pipes.DynamicPipe pipe1(
    ...
    redeclare model HeatTransfer =
        ...
        T_ambient=T_ambient,
        alpha0=hc_air))
    annotation (Placement(transformation(extent={{-30,-114},{-14,-98}})));
  Modelica.Fluid.Pipes.DynamicPipe pipe5(
    ...
    redeclare model HeatTransfer =
        ...
        T_ambient=T_ambient,
        alpha0=hc_air))
    annotation (Placement(transformation(extent={{76,40},{62,54}})));
...
equation
  ...
  connect(pipe2.port_b,PV_049. port_a)
    annotation (Line(points={{58,76},{84,76},{84,72}}, color={0,127,255}));
  ...
  connect(pipe2.port_b, BOP_Mass_flow.port_a)
    annotation (Line(points={{58,76},{96,76}}, color={0,127,255}));
  ...
  connect(T_chiller_before.port_b, pipe7.port_a)
    annotation (Line(points={{214,2},{214,-12},{210,-12}},
                                                 color={0,127,255}));
  connect(pipe7.port_b, T_chiller_after.port_a) annotation (Line(points={{210,-28},
          {210,-40},{214,-40},{214,-52}},
                                     color={0,127,255}));
  ...
  connect(TC_003a.port_b, pipe2.port_a)
    annotation (Line(points={{42,76},{42,76}}, color={0,127,255}));
  connect(Heater_BOP_Demand.y, pump.in_p) annotation (Line(points={{-3.3,-128},
          {-12,-128},{-12,-140.16}},
                                 color={0,0,127}));
  connect(Glycol_HX.port_b_shell, nonLinear_Break_Derivative3.port_a)
    annotation (Line(points={{77,-232.1},{56,-232.1},{56,-232}}, color={0,127,
          255}));
  connect(nonLinear_Break_Derivative3.port_b, PV_012.port_a)
    annotation (Line(points={{56,-220},{56,-188}}, color={0,127,255}));
  connect(TC_006.port_a, nonLinear_Break_Derivative.port_a) annotation (Line(
        points={{16,-145},{16,-146},{12,-146}}, color={0,127,255}));
  connect(nonLinear_Break_Derivative.port_b, pump.port_a)
    annotation (Line(points={{0,-146},{-4,-146}}, color={0,127,255}));
  ...
  connect(nonLinear_Break_Derivative1.port_a, PV_004.port_b) annotation (Line(
        points={{-68,-106},{-78,-106},{-78,-146},{-84,-146}}, color={0,127,255}));
  ...
  connect(Chiller_Mass_flow_T66.port_b, TC_004.port_b) annotation (Line(points={
          ...
          255}));
  connect(nonLinear_Break_Derivative2.port_b, PV_009.port_a)
    annotation (Line(points={{106,-144},{98,-144}}, color={0,127,255}));
  connect(pipe1.port_a, nonLinear_Break_Derivative1.port_b)
    annotation (Line(points={{-30,-106},{-56,-106}}, color={0,127,255}));
  connect(pipe1.port_b, FM_201.port_a)
    annotation (Line(points={{-14,-106},{12,-106}},color={0,127,255}));
  ...
  connect(TC_202.port_a, pipe6.port_b)
    annotation (Line(points={{38,-81},{38,-88},{58,-88}},
                                                 color={0,127,255}));
  connect(pipe6.port_a,PV_051. port_a)
    annotation (Line(points={{70,-88},{70,-106},{62,-106}},
                                                  color={0,127,255}));
  connect(pipe3.port_b, nonLinear_Break_Derivative2.port_a)
    annotation (Line(points={{196,-144},{118,-144}}, color={0,127,255}));
  connect(pipe8.port_b, Chiller_Mass_flow_T66.port_a) annotation (Line(points={{138,
          -186},{124.5,-186},{124.5,-190}},     color={0,127,255}));
  connect(PV_052.port_b, pipe8.port_a) annotation (Line(points={{174,-136},{174,
          -186},{150,-186}}, color={0,127,255}));
  connect(PV_008.port_b, pipe8.port_a) annotation (Line(points={{190,-182},{190,
          -186},{150,-186}}, color={0,127,255}));
  connect(PV_051.port_a, pipe9.port_a) annotation (Line(points={{62,-106},{88,-106}},
                      color={0,127,255}));
  connect(pipe9.port_b, T_ch_o.port_b) annotation (Line(points={{100,-106},{114,
          -106},{114,-105}},              color={0,127,255}));
  connect(pipe5.port_b, m_thot.port_a)
    annotation (Line(points={{62,47},{62,46},{60,46}}, color={0,127,255}));
  connect(PV_049.port_b, pipe5.port_a) annotation (Line(points={{84,60},{84,47},
          {76,47}},                    color={0,127,255}));
  connect(T_discharge_outlet.port_a, pipe10.port_b) annotation (Line(points={{112,47},
          {112,48},{104,48}},                    color={0,127,255}));
  connect(pipe10.port_a, pipe5.port_a) annotation (Line(points={{92,48},{92,47},
          {76,47}},           color={0,127,255}));
  ...
  connect(Sen, control_System_Therminol_4_element_all_modes_ExpTest3_1.ActuatorSubBus)
    annotation (Line(
      points={{-73,142},{19.4667,142},{19.4667,154.15}},
      color={111,216,99},
      pattern=LinePattern.Dash,
      thickness=0.5), Text(
      string="%first",
  ...
  connect(Sen.TC006, TC_006.T) annotation (Line(
      points={{-73,142},{-160,142},{-160,-210},{28,-210},{28,-149.68}},
      color={111,216,99},
      pattern=LinePattern.Dash,
      thickness=0.5), Text(
      string="%first",
  ...
  connect(Sen.Volume_flow_rate, product1.y) annotation (Line(
      points={{-73,142},{-160,142},{-160,-66},{-71,-66}},
      color={111,216,99},
      pattern=LinePattern.Dash,
      thickness=0.5), Text(
      string="%first",
  ...
  connect(T_chiller_after.port_b, nonLinear_Break_Derivative4.port_a)
    annotation (Line(points={{214,-76},{214,-96}}, color={0,127,255}));
  connect(nonLinear_Break_Derivative4.port_b, pipe3.port_a) annotation (Line(
        points={{214,-108},{214,-144},{208,-144}}, color={0,127,255}));
  connect(PV_008.port_a, pipe3.port_b) annotation (Line(points={{190,-170},{190,
          -144},{196,-144}}, color={0,127,255}));
  connect(Ac.PV006, TC_003a.T) annotation (Line(
      points={{-19,142},{-19,140},{30,140},{30,80.32}},
      color={239,82,82},
      pattern=LinePattern.Dash,
      thickness=0.5));
  annotation (
