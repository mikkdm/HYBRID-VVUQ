...
model GHX_UnitSimulation_testModelwBypassLine_v5_NOTWORKING
  ...
  Modelica.Fluid.Sources.MassFlowSource_T Oil_Mass_Flow(
    ...
    T=598.15,
    nPorts=1)
    annotation (Placement(transformation(extent={{102,-12},{86,4}})));
...
equation
  ...
  connect(sensorBus.RejectedHeat, RejectedHeat.y) annotation (Line(
      ...
      pattern=LinePattern.Dash,
      thickness=0.5));
  connect(Temp_TC004.y[1], Oil_Mass_Flow.T_in) annotation (Line(points={{129,
          -20},{110,-20},{110,-0.8},{103.6,-0.8}}, color={0,0,127}));
  connect(actuatorBus.Chiller_Massflow, Chiller_Mass_Flow.m_flow_in)
    annotation (Line(
      points={{30,100},{-102,100},{-102,26.4},{-76,26.4}},
      color={111,216,99},
      pattern=LinePattern.Dash,
      thickness=0.5));
  connect(actuatorBus.PV009_Opening, PV009.opening) annotation (Line(
      points={{30,100},{30,98},{160,98},{160,-88},{8,-88},{8,-54}},
      color={111,216,99},
      pattern=LinePattern.Dash,
      thickness=0.5));
  connect(massflow.y, Oil_Mass_Flow.m_flow_in) annotation (Line(points={{131,12},
          {106,12},{106,2.4},{102,2.4}}, color={0,0,127}));
  annotation (
...
end GHX_UnitSimulation_testModelwBypassLine_v5_NOTWORKING;
