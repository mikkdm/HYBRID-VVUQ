...
model GHX_UnitSimulation_testModelwBypassLine_v4_NOTWORKING

  extends
    NHES.Systems.ExperimentalSystems.TEDS.BaseClasses_1.Partial_SubSystem_A(
    redeclare replaceable ControlSystems.CS_GlycolHX_v5 CS,
  ...
  TRANSFORM.Fluid.Sensors.TemperatureTwoPort Eth_Gly_OutletT(redeclare package
      Medium =
        TRANSFORM.Media.Fluids.EthyleneGlycol.LinearEthyleneGlycol_50_Water,
      precision=3)
    annotation (Placement(transformation(extent={{38,14},{52,26}})));
  ...
  TRANSFORM.Fluid.Sensors.TemperatureTwoPort TC_005(redeclare package Medium =
      ...
      =3) annotation (Placement(transformation(extent={{-24,-12},{-38,4}})));
  ...
  TRANSFORM.Fluid.Sensors.TemperatureTwoPort TC_004(redeclare package Medium =
        TRANSFORM.Media.Fluids.Therminol_66.LinearTherminol66_A_250C, precision
      =3) annotation (Placement(transformation(extent={{38,-10},{52,2}})));
  ...
  TRANSFORM.Fluid.Valves.ValveLinear PV009(
    ...
    dp_nominal=1000,
    m_flow_nominal=2)
    annotation (Placement(transformation(extent={{18,-36},{2,-52}})));
  ...
  TRANSFORM.Fluid.Valves.ValveLinear BypassV_Glycol(
    redeclare package Medium =
        TRANSFORM.Media.Fluids.Therminol_66.LinearTherminol66_A_250C,
    dp_nominal=1000,
    m_flow_nominal=2)
    annotation (Placement(transformation(extent={{2,36},{18,52}})));
...
equation
  ...
  connect(Eth_Gly_OutletT.port_b, boundary1.ports[1])
    annotation (Line(points={{52,20},{90,20}}, color={0,127,255}));
  ...
  connect(sensorBus.Eth_Gly_OutletT, Eth_Gly_OutletT.T) annotation (Line(
      points={{-30,100},{-100,100},{-100,92},{45,92},{45,22.16}},
      color={239,82,82},
      pattern=LinePattern.Dash,
      thickness=0.5));
  connect(TC_005.port_a, Glycol_HX.port_b_tube)
    annotation (Line(points={{-24,-4},{0,-4}},  color={0,127,255}));
  connect(Glycol_HX.port_a_tube, TC_004.port_a)
    annotation (Line(points={{20,-4},{38,-4}}, color={0,127,255}));
  connect(Eth_Gly_OutletT.port_a, Glycol_HX.port_b_shell)
    annotation (Line(points={{38,20},{32,20},{32,0},{20,0},{20,0.6}},
                                                        color={0,127,255}));
  connect(Eth_Gly_InletT.port_b, Glycol_HX.port_a_shell) annotation (Line(
        points={{-24,20},{-12,20},{-12,0.6},{0,0.6}},color={0,127,255}));
  connect(PV009.port_a, TC_004.port_a)
    annotation (Line(points={{18,-44},{32,-44},{32,-4},{38,-4}},
                                                         color={0,127,255}));
  connect(PV009.port_b, TC_005.port_a) annotation (Line(points={{2,-44},{-12,
          -44},{-12,-4},{-24,-4}},
                              color={0,127,255}));
  connect(Oil_Mass_Flow.ports[1], TC_004.port_b)
    annotation (Line(points={{86,-4},{52,-4}}, color={0,127,255}));
  ...
  connect(Eth_Gly_InletT.port_b, BypassV_Glycol.port_a) annotation (Line(points
        ={{-24,20},{-12,20},{-12,44},{2,44}}, color={0,127,255}));
  connect(BypassV_Glycol.port_b, Eth_Gly_OutletT.port_a) annotation (Line(
        points={{18,44},{32,44},{32,20},{38,20}}, color={0,127,255}));
  connect(actuatorBus.Chiller_Massflow, Chiller_Mass_Flow.m_flow_in)
    annotation (Line(
      points={{30,100},{160,100},{160,-92},{-100,-92},{-100,26.4},{-76,26.4}},
      color={111,216,99},
      pattern=LinePattern.Dash,
      thickness=0.5));
  connect(ramp.y, Oil_Mass_Flow.m_flow_in) annotation (Line(points={{129,-20},{
          102,-20},{102,-10.4}}, color={0,0,127}));
  connect(actuatorBus.BypassV_Glycol_Opening, BypassV_Glycol.opening)
    annotation (Line(
      points={{30,100},{160,100},{160,56},{10,56},{10,50.4}},
      color={111,216,99},
      pattern=LinePattern.Dash,
      thickness=0.5));
  connect(actuatorBus.PV009_Opening, PV009.opening) annotation (Line(
      points={{30,100},{160,100},{160,-92},{10,-92},{10,-50.4}},
      color={111,216,99},
      pattern=LinePattern.Dash,
      thickness=0.5));
  annotation (
...
end GHX_UnitSimulation_testModelwBypassLine_v4_NOTWORKING;
