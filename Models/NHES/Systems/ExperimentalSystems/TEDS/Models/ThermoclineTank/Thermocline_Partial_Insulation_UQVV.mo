...
model Thermocline_Partial_Insulation_UQVV "Thermocline Insulation subpackage"
  ...
  Thermocline_Middle_UQVV thermocline_fluidprops_heaters_newHC_120C(
    ...
    dr=geometry.dr,
    T_Init=T_Init)
    annotation (Placement(transformation(extent={{-22,-22},{22,26}})));
  ...
  Thermocline_noInsulation_Bottom_UQVV thermocline_noInsulation_noBeads_Bottom(
    ...
    dr=geometry.dr,
    T_Init=T_Init)
    annotation (Placement(transformation(extent={{-22,-76},{22,-28}})));
  Thermocline_noBeads_Top_UQVV thermocline_noInsulation_noBeads_Top(
    redeclare package Medium = Medium,
    Radius_Tank=geometry.Radius_Tank,
    Porosity=geometry.Porosity,
    XS_fluid=geometry.XS_Fluid,
    ...
    dr=geometry.dr,
    T_Init=T_Init)
    annotation (Placement(transformation(extent={{-22,40},{22,88}})));
  TRANSFORM.HeatAndMassTransfer.Volumes.SimpleWall_Cylinder simpleWall2[TES.nodes](
  ...
  TRANSFORM.HeatAndMassTransfer.BoundaryConditions.Heat.Temperature_multi
    boundary3(nPorts=geometry.nodes, use_port=true)
    annotation (Placement(transformation(
        extent={{-6,-6},{6,6}},
        rotation=90,
        origin={-88,-62})));
  Modelica.Blocks.Sources.RealExpression boundaryT2
                                                  [geometry.nodes](y=fill(
        geometry.T_amb, geometry.nodes)) annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={-88,-80})));
  TRANSFORM.HeatAndMassTransfer.Volumes.SimpleWall_Cylinder simpleWall1[TES.nodes](
    length=fill(thermocline_noInsulation_noBeads_Top.Height_Tank/
        thermocline_noInsulation_noBeads_Top.nodes,
        thermocline_noInsulation_noBeads_Top.nodes),
    r_inner=fill(geometry.Radius_Tank, geometry.nodes),
    r_outer=fill(geometry.Radius_Tank + geometry.Wall_Thickness, geometry.nodes),
    redeclare package Material = WallMaterial,
...
equation
  connect(simpleWall.port_b, thermocline_fluidprops_heaters_newHC_120C.heatPorts[
    :, 1]) annotation (Line(points={{-34,2},{-22.44,2}},
        color={191,0,0}));
  connect(simpleWall.port_a, Insulation.port_b)
    annotation (Line(points={{-54,2},{-60,2}}, color={191,0,0}));
  connect(boundary2.port, Insulation.port_a)
    annotation (Line(points={{-90,-2},{-90,2},{-80,2}},  color={191,0,0}));
  connect(boundaryT.y, boundary2.T_ext)
    annotation (Line(points={{-90,-21},{-90,-10.4}},
                                                   color={0,0,127}));
  connect(thermocline_fluidprops_heaters_newHC_120C.port_b,
    thermocline_noInsulation_noBeads_Bottom.port_a)
    annotation (Line(points={{0,-22},{0,-46.24}}, color={0,127,255}));
  connect(thermocline_noInsulation_noBeads_Bottom.port_b, port_b)
    annotation (Line(points={{0,-57.76},{0,-100}}, color={0,127,255}));
  connect(port_a, thermocline_noInsulation_noBeads_Top.port_a)
    annotation (Line(points={{0,100},{0,70.24}}, color={0,127,255}));
  connect(thermocline_noInsulation_noBeads_Top.port_b,
    thermocline_fluidprops_heaters_newHC_120C.port_a)
    annotation (Line(points={{0,57.76},{0,26}}, color={0,127,255}));
  connect(boundaryT2.y, boundary3.T_ext)
    annotation (Line(points={{-88,-69},{-88,-64.4}}, color={0,0,127}));
  connect(boundary3.port, simpleWall2.port_a)
    annotation (Line(points={{-88,-56},{-88,-52},{-52,-52}}, color={191,0,0}));
  connect(simpleWall2.port_b, thermocline_noInsulation_noBeads_Bottom.heatPorts[
    :, 1]) annotation (Line(points={{-32,-52},{-22.44,-52}}, color={191,0,0}));
  connect(simpleWall1.port_b, thermocline_noInsulation_noBeads_Top.heatPorts[:,
    1]) annotation (Line(points={{-34,64},{-22.44,64}}, color={191,0,0}));
  connect(boundary1.port, simpleWall1.port_a)
    annotation (Line(points={{-92,60},{-92,64},{-54,64}}, color={191,0,0}));
  connect(boundaryT1.y, boundary1.T_ext)
    annotation (Line(points={{-92,45},{-92,51.6}}, color={0,0,127}));
...
end Thermocline_Partial_Insulation_UQVV;
